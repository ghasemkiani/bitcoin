...
class Account extends Obj {
	...
	async toTransfer({
		...
		let script = segwit
			? Script.buildWitnessV0Out(address).toHex()
			: Script.buildPublicKeyHashOut(address).toHex();
		...
		if (agg !== "all") {
			let uu0 = [...utxos].sort((a, b) => b.satoshis - a.satoshis);
			if (agg === "smaller") {
				uu0.reverse();
			}
			let uu = [];
    ...
    tx.from(utxos);
    for (let { address, amount_ } of tos) {
      tx.to(address, amount_);
    }
    tx.change(change);
    let feeEstimates = await client.toGetFeeEstimates();
    let feeRate = feeEstimates["6"];
    let size = utxos.length * 68 + (tos.length + 1) * 31 + 11;
    let fee_ = Math.ceil(size * feeRate);
    tx.fee(fee_);
    tx.sign(account.key);
    let rawTx = tx.serialize();
    let hash = await client.toBroadcast(rawTx);
    return hash;
	}
}
